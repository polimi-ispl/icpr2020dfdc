...
class FaceExtractor:
    ...
    def _untile_detections(self, num_frames:int, frame_size:Tuple[int,int], detections:List[torch.Tensor]) -> List[torch.Tensor]:
    ...
    def _add_margin_to_detections(self, detections:torch.Tensor, frame_size:Tuple[int,int], margin:float=0.2) -> torch.Tensor:
...
class FaceExtractorIspl(FaceExtractor):
    ...
    def process_videos(self, input_dir, filenames, video_idxs):
        ...
        This method is overriden in order to return as another dict field the bounding box of the face
        ...
        This dictionary contains:
            ...
            - faces: a list containing zero or more NumPy arrays with a face crop
            - scores: a list array with the confidence score for each face crop
            - bb: a list containing the coordinates of the bounding box in the frame
        ...
        for v in range(len(tiles)):
            ...
            for i in range(len(detections)):
                ...
                faces, bbs = self._crop_faces(frames[v][i], faces)
    ...
    def process_video_for_bbs(self, video_path):
        """Convenience method for doing face extraction on a single video."""
        input_dir = os.path.dirname(video_path)
        filenames = [os.path.basename(video_path)]
        return self.process_videos_for_bbs(input_dir, filenames, [0])

    def process_videos_for_bbs(self, input_dir, filenames, video_idxs):
        """For the specified selection of videos, grabs one or more frames
        from each video, runs the face detector, and tries to find the faces
        in each frame.
        ...
        Returns a list of dictionaries, one for each frame read from each video, containing the coordinates of
        the bounding box of the face for each frame analyzed.
        ...
        for v in range(len(tiles)):
            ...
            for i in range(len(detections)):
                # Crop the faces out of the original frame.
                faces = self._add_margin_to_detections(detections[i], frame_size, 0.2)
                frame, bbs = self._get_bb_coors(frames[v][i], faces)
    ...
    def _crop_faces(self, frame, detections, face_size=512):
        """Copies the face region(s) from the given frame into a set
        of new NumPy arrays.
        The method is overriden for having a face of fixed size
        ...
        for i in range(len(detections)):
            ymin, xmin, ymax, xmax = detections[i, :4].cpu().numpy().astype(np.int)
            left, top, right, bottom = adapt_bb(frame_height=frame.shape[0], frame_width=frame.shape[1],
                                                bb_height=face_size, bb_width=face_size,
                                                left=xmin, top=ymin, right=xmax, bottom=ymax)
            face = frame[top:bottom, left:right, :]
            faces.append(face)
            bbs.append([xmin - left, ymin - top, xmax - left, ymax - top])
        return faces, bbs

    def _get_bb_coors(self, frame, detections):
        """Gets the global bb coordinates in the frame
    ...
    def keep_only_best_bb(self, crops):
        """For each frame, only keeps the bb with the highest confidence.
        ...
        for i in range(len(crops)):
            frame_data = crops[i]
            if len(frame_data["bbs"]) > 0:
                frame_data['bbs'] = frame_data['bbs'][:1]
